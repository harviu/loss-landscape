...
def train(trainloader, net, criterion, optimizer, use_cuda=True):
    ...
    if isinstance(criterion, nn.CrossEntropyLoss):
        ...
        return total_recon_loss/total, total_kl_loss/total
...
if __name__ == '__main__':
    ...
    if not args.resume_model:
        if args.model != "vae":
            ...
            recon_loss, kl_loss = train(trainloader, net, criterion, optimizer, use_cuda)
            test_recon_loss, test_kl_loss = test(testloader, net, criterion, use_cuda)
            status = 'e: %d recon_loss: %.5f kl_loss: %.5f test_recon_loss %.5f test_recon_loss %.5f \n' % (0, recon_loss,kl_loss,test_recon_loss, test_kl_loss)
            state = {
                'epoch': 0,
                'state_dict': net.module.state_dict() if args.ngpu > 1 else net.state_dict()
            }
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        if args.model != "vae":
            ...
            recon_loss, kl_loss = train(trainloader, net, criterion, optimizer, use_cuda)
            test_recon_loss, test_kl_loss = test(testloader, net, criterion, use_cuda)
            status = 'e: %d recon_loss: %.5f kl_loss: %.5f test_recon_loss %.5f test_recon_loss %.5f \n' % (epoch, recon_loss,kl_loss,test_recon_loss, test_kl_loss)
        print(status)
        f.write(status)
